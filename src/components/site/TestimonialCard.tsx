...
export function TestimonialCard({ t, accent = "ivory" }: { t: Testimonial; accent?: "ivory" | "peach" | "mint" }) {
  const bg = accent === "peach" ? "bg-[var(--peach)]" : accent === "mint" ? "bg-[var(--mint)]" : "bg-ivory";
  return (
    <figure className={`relative flex h-full flex-col gap-6 overflow-hidden rounded-3xl ${bg} p-8 shadow-soft`}>
